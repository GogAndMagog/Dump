...
public class CompletableFutureDemo {
    ...
    public List<URL> getImageURLs(String webpage) // not timeconsuming
    {
        try {
    ...
    public void saveImages(List<BufferedImage> images) {
        ...
        try {
            for (int i = 0; i < images.size(); i++) {
                String filename = "C:\\Users\\user\\Downloads\\image" + (i + 1) + ".png";
...
HttpClient client = HttpClient.newBuilder().executor(executor).build();
        ...
        try {
            request = HttpRequest.newBuilder(urlToProcess.toURI()).GET()
            .build();
